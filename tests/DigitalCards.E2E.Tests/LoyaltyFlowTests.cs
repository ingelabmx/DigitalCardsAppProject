...
public sealed class LoyaltyFlowTests : IClassFixture<WebAppFixture>
{
    ...
    public async Task ClientBusinessGoogleWalletAndStampFlow_WorksWithFakeServices()
    {
        ...
        var userName = $"maria-{Guid.NewGuid():N}"[..18];
        ...
        await page.GetByTestId("register-email").FillAsync($"{userName}@example.test");
        ...
        await page.GetByTestId("business-email").FillAsync("demo@digitalcards.test");
        await page.GetByTestId("business-password").FillAsync("business123");
        await page.GetByTestId("business-login-submit").ClickAsync();
        Assert.Contains("Demo Coffee", await page.GetByTestId("business-dashboard-title").InnerTextAsync());
        ...
        Assert.Contains("Demo Coffee", await page.GetByTestId("wallet-select").InnerTextAsync());
    ...
    public async Task WalletLanding_IsResponsiveForIPhoneAndAndroidViewports()
    {
        ...
        await using var browser = await playwright.Chromium.LaunchAsync(new BrowserTypeLaunchOptions { Headless = true });
        var page = await browser.NewPageAsync();
        var userName = $"phone-{Guid.NewGuid():N}"[..18];
